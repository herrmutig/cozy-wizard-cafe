...
# TODO add a Timer that mimics guest chilling at table and talking a bit.. 
# TODO (maybe add random sprechblasen)
func _ready():
...
func take_cup(cup_to_consume:Cup) -> bool:
	if (cup != null || has_order == false):
		print("guest already has a cup or order")
		return false
	
	if check_ingridients(cup_to_consume.ingridients):
		print("guest consumed cup, and is happy")
	else:
		print("guest consumed cup, and is unhappy")
